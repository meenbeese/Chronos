...
class HomeFragment : BaseFragment() {
    ...
    private fun invokeAlarmScheduler() {
        ...
        picker.addOnPositiveButtonClickListener {
            ...
            alarm.time[Calendar.HOUR_OF_DAY] = picker.hour
            alarm.time[Calendar.MINUTE] = picker.minute
            alarm.setTime(chronos, manager, alarm.time.timeInMillis)
            alarm.setEnabled(context, manager, true)
            chronos?.onAlarmsChanged()
        }

        picker.show((context as FragmentActivity).supportFragmentManager, "MATERIAL_TIME_PICKER")
    }
